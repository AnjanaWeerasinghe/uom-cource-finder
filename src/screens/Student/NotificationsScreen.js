...
export default function NotificationsScreen({ navigation }) {
  ...
  // Generate notifications from submissions and enrollments
  const notifications = [];

  // Graded submissions notifications
  mySubmissions
    ...
    .forEach(sub => {
      notifications.push({
        ...
        message: `Your submission for "${sub.workTitle}" has been graded`,
        detail: `Grade: ${sub.grade}/100`,
        icon: 'check-circle',
        color: '#10b981',
        bgColor: '#dcfce7',
        time: new Date(sub.gradedAt),
        action: () => navigation.navigate('MySubmissions')
      });
    });
  ...
  mySubmissions
    ...
    .forEach(sub => {
      notifications.push({
        ...
        message: `Your submission for "${sub.workTitle}" is being reviewed`,
        detail: 'Waiting for grade',
        icon: 'clock',
        color: '#f59e0b',
        bgColor: '#fef3c7',
  ...
  enrollments.slice(0, 5).forEach(enroll => {
    notifications.push({
      ...
      message: `You've been enrolled in "${enroll.courseTitle}"`,
      detail: 'Start learning now!',
      icon: 'book-open',
      color: '#3b82f6',
      bgColor: '#dbeafe',
      time: new Date(enroll.enrolledAt),
      action: () => navigation.navigate('Enrolled')
    });
  });
